...
function buildButtons(matchId, isOpen) {
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`bet_${matchId}_1`).setLabel('🤙 Tranquilo').setStyle(ButtonStyle.Secondary).setDisabled(!isOpen),
    new ButtonBuilder().setCustomId(`bet_${matchId}_2`).setLabel('⚡ Jogador').setStyle(ButtonStyle.Primary).setDisabled(!isOpen),
    new ButtonBuilder().setCustomId(`bet_${matchId}_3`).setLabel('🔥 Mesmo jogador').setStyle(ButtonStyle.Danger).setDisabled(!isOpen),
  );
  return row;
}
